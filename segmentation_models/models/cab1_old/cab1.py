...
def AM(X, channel, final_channel, activation, fused_layers, encoder_threshold, ratio=8, use_hhdc=False, use_cam=False, name=''):
    if use_hhdc == 6:
    ...
    X = CONV_stack(X, final_channel, kernel_size=3, stack_num=1,
                   activation=activation, batch_norm=True, name='{}_am_conv'.format(name),
                   enhance_skeleton=True, direction_aware=False)
    return X
...
class CAM(tf.keras.layers.Layer):
    def __init__(self, out_planes, ratio=2):
        ...
        self.relu1 = tf.keras.layers.Activation('relu')  # TODO maybe use this line instead of TLU
...
def unet_3plus_2d_base(input_tensor, filter_num_down, filter_num_skip, filter_num_aggregate,
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    for i in range(depth_decode):
        ...
        X = AM(X, channel=filter_num_aggregate, final_channel=filter_num_down[-i-2], activation=activation, fused_layers=X_fscale,
