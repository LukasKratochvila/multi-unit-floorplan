...
class ChannelAttention(tf.keras.layers.Layer):
    def __init__(self, in_planes, out_planes, ratio=2):
        ...
        self.use_hhdc = False
        if self.use_hhdc:
            self.hhdc = HHDC(in_planes, out_planes)
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if backbone is None:
        ...
        X = CONV_stack(X, filter_num_down[0], kernel_size=3, stack_num=stack_num_down,
                       ...
                       enhance_skeleton=True, direction_aware=True)
        X_encoder.append(X)
...
def ours_multi(input_size, n_labels, filter_num_down, filter_num_skip='auto', filter_num_aggregate='auto',
               stack_num_down=2, stack_num_up=1, activation='ReLU', output_activation='Sigmoid',
               batch_norm=False, pool=True, unpool=False, deep_supervision=False,
               backbone=None, weights='imagenet', freeze_backbone=True, freeze_batch_norm=True, name='unet3plus',
               aaf=False):
    '''
