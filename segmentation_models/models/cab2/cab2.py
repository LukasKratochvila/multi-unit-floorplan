...
from segmentation_models.base.layer_utils import CONV_stack, decode_layer, encode_layer, CONV_output
from segmentation_models.models.unet3plus.model_unet_2d import UNET_left, UNET_right

from segmentation_models import backbone_zoo
...
def AM(X, channel, activation, fused_layers, encoder_threshold, ratio=8, use_hhdc=False, use_cam=False, name=''):
    ...
    if use_cam == 3:
        spatial_feature = SAM3(channel_s, False)(X_s)
        # if use_cam == 2:
...
class HHDC2(tf.keras.layers.Layer):
    def __init__(self, out_planes, concat=False, dilations=None):
        super(HHDC2, self).__init__()
        self.concat = concat
        if concat:
            out_planes -= 2
        self.conv1 = Conv2D(out_planes, 3, use_bias=False, padding='same')
        ...
        self.convd4 = Conv2D(out_planes, 3, dilation_rate=4, use_bias=False, padding='same')

    def call(self, x):
        add1 = self.conv1(x)
        add2 = add1 + self.convd1(x)
        add3 = add2 + self.convd2(x)
        add4 = add3 + self.convd4(x)
        out = add1 + add2 + add3 + add4
        if self.concat:
...
def cab2(input_size, n_labels, filter_num_down, filter_num_skip='auto', filter_num_aggregate='auto',
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if deep_supervision:
        ...
        for i in range(1, L_out):
            ...
            # X = decode_layer(X, n_labels, pool_size, 'nearest',
