...
def CONV_stack(X, channel, kernel_size=3, stack_num=2,
               ...
               square_conv=True, enhance_skeleton=False,
               direction_aware=False, groups=16, use_bn=False):
    '''
    ...
    for i in range(stack_num):
        ...
        if square_conv:
            # square_conv
            A = Conv2D(channel, (kernel_size, kernel_size), padding='same', use_bias=bias_flag,
                       dilation_rate=dilation_rate, name='{}_{}'.format(name, i))(X)
            T = A

        if enhance_skeleton:
            # horizontal_conv
            C = Conv2D(channel, (1, kernel_size), padding='same', use_bias=bias_flag,
        ...
        if direction_aware:
            D1 = Conv2D(channel, kernel_size, padding='same', use_bias=bias_flag,
        ...
        if batch_norm:
            if use_bn and channel > 16:
                ...
                X = tfa.layers.GroupNormalization(axis=3, groups=groups, name='{}_{}_gn'.format(name, i))(X)
            # X = tfa.layers.FilterResponseNormalization(name='{}_{}_frn'.format(name, i))(X)
