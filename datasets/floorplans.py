...
def _parse_function(example_proto, classes, n_upsample, reduction_ratio):
    ...
    img, mask = preprocess(image, mask, heatmaps, width, height, classes)
    return shape_adjustment_transform(img, mask, height, width, n_upsample, reduction_ratio)


def _parse_function_normalize(example_proto, classes, n_upsample, reduction_ratio):
    feature = {'image': tf.io.FixedLenFeature([], tf.string),
               'mask': tf.io.FixedLenFeature([], tf.string),
               'heatmaps': tf.io.FixedLenFeature([], tf.string),
               'width': tf.io.FixedLenFeature([], tf.int64),
    ...
    image, mask, heatmaps, width, height = decode_all_raw(example)
    img, mask = preprocess_normalize(image, mask, heatmaps, width, height, classes)
    return shape_adjustment_transform(img, mask, height, width, n_upsample, reduction_ratio)
...
def preprocess(img, mask, heatmaps, width, height, classes):
    ...
    mask = tf.one_hot(mask, depth=len(classes))
    mask = tf.concat([mask, heatmaps], axis=-1)

    return img, mask


def preprocess_normalize(img, mask, heatmaps, width, height, classes):
    img = tf.cast(img, dtype=tf.float32)
...
def create_dataset(datasets, type='default', input_dirs='./', output_dir='./', data_dir='annotations/hdd/'):
    ...
    print('Creating dataset from: ', datasets)
...
def write_record(paths, name, type, data_dir='annotations/hdd/'):
    ...
    print('Creating tfrecord: ', name, ' with: ', len(paths), ' samples.')
    for path in tqdm(paths, 'Processing'):
        # Load the image
        image, mask, heatmaps = load_images(path, type, data_dir=data_dir)
        if type == 'cubicasa5k':
            # Compress labels
            # From  ['walls', 'glass_walls', 'railings', 'doors', 'sliding_doors', 'windows', 'stairs_all']
            # To    ['walls', 'railings', 'doors', 'windows', 'stairs_all']
            # To    ['walls', 'glass_walls'+'railings', 'doors', 'windows', 'sliding_doors'+'stairs_all']
...
def load_images(path, type='', data_dir='annotations/hdd/', show=False):

    images = path.split('\t')

    image = cv2.imread(os.path.join(data_dir, images[0])).astype(np.uint8)
    mask = cv2.imread(os.path.join(data_dir, images[1]), 0).astype(np.uint8)
    ...
    if type == 'combi' and len(images[2:]) < 3:
        ...
        heatmaps = [
            cv2.resize(cv2.imread(os.path.join(data_dir, images[2]), 0).astype(np.uint8), (image.shape[1], image.shape[0])),
            np.zeros((image.shape[0], image.shape[1]), np.uint8),
            cv2.resize(cv2.imread(os.path.join(data_dir, images[3]), 0).astype(np.uint8), (image.shape[1], image.shape[0])),
        ]
    else:
        heatmaps = []
        for heatmap in images[2:]:
            heatmaps.append(
                cv2.resize(cv2.imread(os.path.join(data_dir, heatmap), 0).astype(np.uint8), (image.shape[1], image.shape[0])))
        if len(heatmaps) == 0:
    ...
    if show:
        print('Showing: ', images[0].split('/')[1])
        plt.figure(dpi=400)
...
''' Function for shape adjustment on dataset w.r.o. model
    Xnet does not work with some shapes 
    (shape must be divided by number of upsampling blocks)'''
def shape_adjustment_transform(img, mask, height, width, n_upsample_blocks=5, reduction_ratio=None):
    if reduction_ratio is not None:
        height = tf.math.floordiv(height, reduction_ratio)
    ...
    if n_upsample_blocks is not None:
        w_r = tf.math.floormod(width, tf.math.pow(2, n_upsample_blocks))
        h_r = tf.math.floormod(height, tf.math.pow(2, n_upsample_blocks))
        if w_r != 0 or h_r != 0:
            width = width - w_r
            height = height - h_r
            if width > 0 and height > 0:
    ...
    w_i, h_i = img.shape[:2]
    w_m, h_m = mask.shape[:2]
    if h_i != h_m or w_i != w_m:
        mask = tf.image.resize(mask, [h_i, w_i])
