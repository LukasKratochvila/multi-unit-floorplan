...
from datasets.dataset_statistics import get_all_paths
...
def augmentation(dataset, input_dir='data/', output_dir='data/r3d_augment', ann_dir='annotations/'):
    print("Starting augmentation")

    sizes = list(range(MIN_SIZE, MAX_SIZE + STRIDE, STRIDE))

    paths = get_all_paths(dataset, data_dir=ann_dir)
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)

    pool = Pool(processes=10)
    pool.map(partial(process, input_dir, output_dir, sizes), paths)
    pool.close()
    pool.join()


def process(data_dir, output_dir, sizes, p):

    i = p.split('\t')
    n = os.path.basename(os.path.dirname(i[0]))
    input_org = cv2.imread(os.path.join(data_dir, i[0]))
    mask_org = cv2.imread(os.path.join(data_dir, i[1]), 0)
