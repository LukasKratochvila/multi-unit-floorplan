...
from utils import ind2rgb, ind2rgba, rgb2ind
...
def main():
    ...
    for model, normalize in models:
        ...
        if predict:
            ...
            mplan_s2 = cv2.imread('resources/multi_plan2.jpg')
            ...
            images = [single, mplan_s, mplan_s2]
            for i, image_org in enumerate(images):
                ...
                result = unpad(result, pads)
                result_pp = unpad(result_pp, pads)
            ...
            for (image, label) in test_dataset.skip(0).take(25).batch(1):
                ...
                write('label_' + str(i), model_name,
                      ind2rgb(label[0, :, :, :len(classes)].numpy().argmax(axis=-1), conversion=conversion))
