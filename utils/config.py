...
def get_args_dict(args):
    """
    Function for creation args dictionary from parser args
    If value is None, -1, False - assume that value did not enter
    """
    args_dict = dict()
    for arg in vars(args):
        value = getattr(args, arg)
        if value is not None and not value == -1 and value:
            # parse inner dict
            if '.' in arg:
                primary_key = arg.split('.')[0]
                last_key = arg.split('.')[-1]
                inner_dict = {last_key: value}
                for key in reversed(arg.split('.')[1:-1]):
                    inner_dict = {key: inner_dict}
                args_dict[primary_key] = inner_dict
            else:
                args_dict[arg] = value
    return args_dict
...
def add_args(parser, cfg, prefix=''):
    for k, v in cfg.items():
        if k == 'cfg_options':
            parser.add_argument('--' + prefix + k, nargs='+', action=DictAction,
            ...
            parser.add_argument('--' + prefix + k, help=f'{v}')
        elif isinstance(v, bool):
            parser.add_argument('--' + prefix + k, action='store_true', help=f'Store true value')
        elif isinstance(v, int):
            parser.add_argument('--' + prefix + k, type=int, default=-1)
        elif isinstance(v, float):
            parser.add_argument('--' + prefix + k, type=float, default=-1)
...
class Config:
    ...
    def _file2dict(filename, use_predefined_variables=True):
        ...
        if not os.path.exists(filename):
            ValueError(f'Filename {filename} does not exists!')
        ...
        with tempfile.TemporaryDirectory() as temp_config_dir:
            ...
            if filename.endswith('.py'):
                ...
                cfg_dict = utils.load(temp_config_file.name)
            # close temp file
            temp_config_file.close()
    ...
    def dump(self, file=None):
        ...
        if file is None:
            if self.filename is None or self.filename.endswith('.py'):
                ...
                return utils.dump(cfg_dict, file_format=file_format)
        elif file.endswith('.py'):
            with open(file, 'w', encoding='utf-8') as f:
                f.write(self.pretty_text)
        else:
            file_format = file.split('.')[-1]
            return utils.dump(cfg_dict, file=file, file_format=file_format)
    ...
    def to_dict(self):
        return super().__getattribute__('_cfg_dict')
